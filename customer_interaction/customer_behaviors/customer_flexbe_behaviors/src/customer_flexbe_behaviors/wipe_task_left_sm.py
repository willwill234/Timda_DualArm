...
from customer_flexbe_states.get_spray_alcohol_II import GetSprayAlcoholPoseII
from customer_flexbe_states.get_spray_alcohol_pose import GetSprayAlcoholPose
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('arrive_bottle',
										FixedPoseMoveState(robot_name='left_arm', en_sim=self.en_sim, mode='p2p', speed=35, pos=[-0.1166, 0.1042, -0.6700], euler=[-47.024, 3.005, -44.998], phi=0),
										transitions={'done': 'grab_bottle', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:783 y:309
			OperatableStateMachine.add('arrive_return_bottle',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=20, pos=[-0.1156, 0.1462, -0.6710], euler=[-44.024, -0.005, -44.998], phi=0),
			...
			OperatableStateMachine.add('init_left_arm',
										InitRobotState(robot_name='left_arm', en_sim=self.en_sim, speed=20),
			...
			OperatableStateMachine.add('move_left_arm',
										customer_flexbe_states__IKMoveState(robot_name='left_arm', en_sim=self.en_sim),
			...
			OperatableStateMachine.add('return_bottle',
										FixedPoseMoveState(robot_name='left_arm', en_sim=self.en_sim, mode='line', speed=35, pos=[-0.1156, 0.1462, -0.5500], euler=[-44.024, -0.005, -44.998], phi=0),
