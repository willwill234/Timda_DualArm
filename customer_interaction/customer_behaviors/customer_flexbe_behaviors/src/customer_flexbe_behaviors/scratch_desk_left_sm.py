...
from customer_flexbe_states.get_scratch_pose import GetScratchPose
...
@author: Luis
'''
class scratch_desk_leftSM(Behavior):
	'''
	Sample behavior of left arm scratch desk
	'''


	def __init__(self):
		super(scratch_desk_leftSM, self).__init__()
		self.name = 'scratch_desk_left'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('initRobot',
										InitRobotState(robot_name=self.robot_name, en_sim=self.en_sim, speed=20),
										transitions={'done': 'gripper_reset', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:266 y:408
			OperatableStateMachine.add('above_scratcher',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=50, pos=[-0.255, 0.1363, -0.50500], euler=[-45.0, 0.0, 0.0], phi=0),
										transitions={'done': 'get_scratch_pose', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:517 y:292
			OperatableStateMachine.add('approach_return_spot',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=20, pos=[-0.255, 0.1363, -0.50500], euler=[-45.0, 0.0, 0.0], phi=0),
										transitions={'done': 'return_scratcher', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:269 y:170
			OperatableStateMachine.add('approach_scratcher',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=50, pos=[-0.255, 0.2863, -0.65000], euler=[-45.0, 0.0, 0.0], phi=0),
										transitions={'done': 'arrive_scratcher', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:270 y:260
			OperatableStateMachine.add('arrive_scratcher',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=20, pos=[-0.255, 0.1363, -0.75000], euler=[-45.0, 0.0, 0.0], phi=0),
			...
			OperatableStateMachine.add('return_scratcher',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=20, pos=[-0.255, 0.1363, -0.7400], euler=[-45.0, 0.0, 0.0], phi=0),
										transitions={'done': 'gripper_release', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:999 y:195
			OperatableStateMachine.add('safety_back',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=20, pos=[-0.255, 0.2863, -0.65000], euler=[-42.024, 0.005, 4.498], phi=0),
