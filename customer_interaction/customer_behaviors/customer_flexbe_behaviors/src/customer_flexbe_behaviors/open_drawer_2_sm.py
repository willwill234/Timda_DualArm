...
@author: Luis
'''
class open_drawer_2SM(Behavior):
	'''
	example for dual  arm to open drawer level2
	'''


	def __init__(self):
		super(open_drawer_2SM, self).__init__()
		self.name = 'open_drawer_2'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('prepare_pose',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='p2p', speed=self.default_speed, pos=[0.5, -0.126, -0.285], euler=[48.0, 90.0, 0.0], phi=0),
										transitions={'done': 'arrive_drawer', 'failed': 'failed'},
										autonomy={'done': Autonomy.Off, 'failed': Autonomy.Off})

			# x:503 y:217
			OperatableStateMachine.add('pull_drawer',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=self.default_speed, pos=[0.5, -0.216, -0.285], euler=[48.0, 90.0, 0.0], phi=0),
			...
			OperatableStateMachine.add('safety_back',
										FixedPoseMoveState(robot_name='right_arm', en_sim=self.en_sim, mode='line', speed=self.default_speed, pos=[0.4, -0.216, -0.285], euler=[48.0, 90.0, 0.0], phi=0),
			...
			OperatableStateMachine.add('arrive_drawer',
										FixedPoseMoveState(robot_name=self.robot_name, en_sim=self.en_sim, mode='line', speed=self.default_speed, pos=[0.7, -0.216, -0.285], euler=[48.0, 90.0, 0.0], phi=0),
