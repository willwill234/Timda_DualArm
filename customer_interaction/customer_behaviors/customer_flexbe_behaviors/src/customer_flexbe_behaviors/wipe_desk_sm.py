...
from customer_flexbe_states.get_wipe_pose import GetWipePose
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('arrive_bottle',
										FixedPoseMoveState(robot_name='left_arm', en_sim=self.en_sim, mode='p2p', speed=35, pos=[-0.1146, 0.1192, -0.6770], euler=[-47.024, 3.005, -44.998], phi=0),
			...
			OperatableStateMachine.add('init_right_arm',
										InitRobotState(robot_name='right_arm', en_sim=self.en_sim, speed=20),
			...
			OperatableStateMachine.add('move_right_arm',
										customer_flexbe_states__IKMoveState(robot_name='right_arm', en_sim=self.en_sim),
